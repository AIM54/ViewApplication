 ...
 * {@link Callback} class or implementing {@link android.support.v7.widget.helper.ItemTouchHelper.ViewDropHandler}
...
public class MyItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    private final RecyclerView.OnItemTouchListener mOnItemTouchListener = new RecyclerView.OnItemTouchListener() {
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            } else if (mSelected == null && action == MotionEvent.ACTION_MOVE && animation != null) {
                float touchX = event.getX();
                float touchY = event.getY();
                float distanceX = touchX - mInitialTouchX;
                float distanceY = touchY - mInitialTouchY;
                if (Math.abs(distanceX) > Math.abs(distanceY) && Math.abs(distanceX) > rightMenuWidth / 10) {
    ...
    public void onChildViewDetachedFromWindow(View view) {
        ...
        CommonLog.i("onChildDetachedFromWindow");
        view.scrollTo(0, 0);
        if (holder == null) {
            return;
        }
    ...
    public abstract static class Callback {
        ...
        public void onMoved(final RecyclerView recyclerView,
            ...
            if (layoutManager instanceof android.support.v7.widget.helper.ItemTouchHelper.ViewDropHandler) {
                ((android.support.v7.widget.helper.ItemTouchHelper.ViewDropHandler) layoutManager).prepareForDrop(viewHolder.itemView,
                        target.itemView, x, y);
                return;
            }
